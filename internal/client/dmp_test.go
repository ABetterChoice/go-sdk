...
func TestIsHitDMP(t *testing.T) {
	batchGetDMPTagResultURI = mockDMPServer(t).URL
	RegisterDMPClient(&tabDMPClient{
	...
	type args struct {
		ctx    context.Context
		req    *protocdmpproxyserver.GetDMPTagResultReq
		dmpTag string
	}
	tests := []struct {
		name    string
		...
		},
		{
			...
			args: args{
				...
				req: &protocdmpproxyserver.GetDMPTagResultReq{
					...
					DmpPlatformCode: 3,
					TagList:         []string{"dmpCodeTest1", "dmpCodeTest2"},
				},
				dmpTag: "dmpCodeTest1",
			},
			want:    true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := IsHitDMP(tt.args.ctx, tt.args.req, tt.args.dmpTag)
			if (err != nil) != tt.wantErr {
				t.Errorf("IsHitDMP() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("IsHitDMP() got = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestIsHitDMPFailure(t *testing.T) {
